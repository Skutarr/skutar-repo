...
class Pyromancer(Wizard):  # наследование
    ...
    def set(self, wizard_name, specialty, magic_skill, mp_lvl,fire_skill):
        self.wizard_name = wizard_name
        self.specialty = specialty
        self.magic_skill = magic_skill
        self.mp_lvl = mp_lvl
...
igor = Wizard("Igor_simpleMag", "first_class_summoner", "frog_summon", 30) # либо установить параметры через конструктор
# igor.set("Igor_simpleMag", "first_class_summoner", "frog_summon", 25) # либо через сеттер
igor.info()

archi = Pyromancer("Archi_FireManiac","first_class_fireEater","upper_fireball",40)
archi.info()
